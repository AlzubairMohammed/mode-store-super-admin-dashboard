<template>
  <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
      ...
      <tbody>
        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700" :key="subCategory.id" v-for="(subCategory, index) in allSubSections" v-on="join(subCategory.sections_id)">
          <th
            scope="row"
            class="
              px-6
...
export default {
    ...
    methods:{
      join(sectionId){
        this.info = this.allCategories.filter((allCategories)=>{
          return allCategories.id === sectionId
        })
        this.section = this.info[0]
      },
